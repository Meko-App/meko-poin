...
class CardPlastikTerjual extends StatefulWidget {
  final MasterDataRepository masterDataRepo;
  final TransactionItemRepository transactionItemRepo;

  const CardPlastikTerjual(
      {super.key,
      required this.masterDataRepo,
      required this.transactionItemRepo});

  @override
  State<CardPlastikTerjual> createState() => _CardPlastikTerjualState();
}

class _CardPlastikTerjualState extends State<CardPlastikTerjual> {
  int currentPage = 1;
  ...
  Future<void> _loadSummaryData() async {
    try {
      ...
      final filteredMasterData =
          masterDataList.where((data) => data.category == 'Packaging').toList();
          ...
          if (summaryData.length > 4)
            Container(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    '$startItem-$endItem of ${summaryData.length}',
                    style: const TextStyle(
                        fontSize: 13,
                        height: 14 / 13,
                        color: CustomColors.fontSubColor,
