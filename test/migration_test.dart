...
void main() {
  ...
  test('Migrate real production v1 database to v14', () async {
    final src = File(
...
// hasn't been migrated yet).
    ...
    expect(version, 14, reason: 'should be upgraded to v14');
    ...
    expect(kasCols.contains('variable'), true);
    expect(kasCols.contains('category_id'), true);
    expect(kasCols.contains('from_variable'), true);
    expect(kasCols.contains('to_variable'), true);

    final invLogCols = await cols('Data_Inventory_Log');
    expect(invLogCols.contains('transaction_id'), true);
    ...
    // All legacy kas rows must have a variable (backfilled).
    final kasNoVariable = await db.rawQuery(
        "SELECT COUNT(*) AS c FROM Data_Kas WHERE variable IS NULL OR variable = ''");
    expect(kasNoVariable.first['c'], 0);
